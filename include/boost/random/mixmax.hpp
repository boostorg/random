 ...
 std::uint64_t operator()()    ///< return one uint64 between min=0 and max=2^61-1
 {
    ...
    operator>> (std::basic_istream<CharT,Traits> &in, mixmax_engine& me){
        ...
        if(std::getline( in, line)){
                ...
                vec[Ndim-1]=std::stoull(token);
                sum=me.MOD_MERSENNE(sum+vec[Ndim-1]);
                if ( iss.fail() ) {in.setstate(std::ios::failbit);}
            }
...
//                        <<counter<<" "<<sumtmp<<" vs "<<sum<<"\n";
    ...
    friend bool operator==(const mixmax_engine & x,
                           const mixmax_engine & y){return x.S.counter==y.S.counter && x.S.sumtot==y.S.sumtot && x.S.V==y.S.V ;}; // && x.counter == y.counter;
    friend bool operator!=(const mixmax_engine & x,
                           const mixmax_engine & y){return !operator==(x,y);}; // && x.counter == y.counter
...
#if defined(BOOST_HAS_INT128)
template <int Ndim, unsigned int SPECIALMUL, std::int64_t SPECIAL> inline std::uint64_t mixmax_engine  <Ndim, SPECIALMUL, SPECIAL> ::mod128(__uint128_t s){
    std::uint64_t s1;
    s1 = ( (  ((std::uint64_t)s)&M61 )    + (  ((std::uint64_t)(s>>64)) * 8 )  + ( ((std::uint64_t)s) >>BITS) );
    return	MOD_MERSENNE(s1);
}

template <int Ndim, unsigned int SPECIALMUL, std::int64_t SPECIAL> inline std::uint64_t mixmax_engine  <Ndim, SPECIALMUL, SPECIAL> ::fmodmulM61(std::uint64_t cum, std::uint64_t a, std::uint64_t b){
    __uint128_t temp;
...
template <int Ndim, unsigned int SPECIALMUL, std::int64_t SPECIAL> mixmax_engine  <Ndim, SPECIALMUL, SPECIAL> & mixmax_engine  <Ndim, SPECIALMUL, SPECIAL> ::operator=(const mixmax_engine& other ){
    S.V = other.S.V;
