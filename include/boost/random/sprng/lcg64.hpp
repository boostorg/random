...
//  BOOST_PP_REPEAT_FROM_TO(1, 5, BOOST_RANDOM_LCG64_CONSTRUCTOR)
  ...
  void seed_implementation(unsigned int stream=0, unsigned int num_stream=1, int s=0, unsigned int param=0)
  {
    ...
    sprng_ptr= lcg64_init_rng (sprng_type, stream, num_stream, seed, param);
    if (sprng_ptr==0)
  ...
  BOOST_PARAMETER_MEMFUN(void,1,4,seed,parallel_seed_params)
  {
    seed_implementation(p[stream_number|0u], p[total_streams|0u], p[global_seed|0], p[parameter|0u]);
  }
  
  /// seed with a range of unsigned int values
  template<class It>
  void seed(It const& first, It const& last)
  {
    if(first == last)
  ...
  static bool validation(IntType x) 
  {
